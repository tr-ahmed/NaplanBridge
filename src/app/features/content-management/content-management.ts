...
private async addEntity(): Promise<void> {
  switch (this.entityType) {
    ...
    case 'lesson': {
      const { title, videoUrl, description, weekId, duration, objectives, posterUrl } = this.form;
      if (!title || !videoUrl || !description || weekId == null) {
        Swal.fire('Error', 'Please fill all required lesson fields.', 'error');
        throw new Error('Validation failed');
      }

      const payload = { title, videoUrl, description, weekId, duration, objectives, posterUrl };
