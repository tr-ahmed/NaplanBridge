...
export class StudentTutoringComponent implements OnInit {
  ...
  sessions = signal<PrivateTutoringDto[]>([]);
   ...
   */
  canJoinSession(session: PrivateTutoringDto): boolean {
    const now = new Date();
   ...
   */
  getSessionStatus(session: PrivateTutoringDto): 'past' | 'starting-soon' | 'upcoming' | 'scheduled' {
    const now = new Date();
    const sessionTime = new Date(session.scheduledDateTime);
    const minutesDiff = Math.floor((sessionTime.getTime() - now.getTime()) / 1000 / 60);
   ...
   */
  joinSession(session: PrivateTutoringDto): void {
